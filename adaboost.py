...
data={}
model = make_pipeline(StandardScaler(),AdaBoostRegressor(base_estimator=LinearRegression(),random_state=10)) #pipeline to make line regression model 
model.fit(train_X,train_y.values.ravel()) #only for adaboost and other boosting classifiers
...
f.write("Adaboost DT - RMSE obtained ="+str(error))
print("RMSE :",error)
...
data["AdaBoost"]=pred_delivery_date
...
compare.to_csv("AdaBoost.csv",sep="\t") #saving the csv
